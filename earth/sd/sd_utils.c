...
void sdinit() {
    ...
    while (reply != 0x01) reply = busy_recv_byte();
    while (busy_recv_byte() != 0xFF);
    ...
    if (reply & 0x04) {
        ...
        uint payload;
        for (uint i = 0; i < 4; i++)
            ((char*)&payload)[3 - i] = busy_recv_byte();
    ...
    if (SD_CARD_TYPE == SD_TYPE_SD2) {
        ...
        sd_exec_cmd(cmd58);
        for (uint i = 0; i < 4; i++) payload[3 - i] = busy_recv_byte();
